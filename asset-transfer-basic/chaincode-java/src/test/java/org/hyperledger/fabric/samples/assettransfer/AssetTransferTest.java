...
public final class AssetTransferTest {
    ...
    class InvokeDuplicateAssetTransaction {
        ...
        public void whenAssetExistsAndDuplicable() {
            ...
            when(ctx.getStub()).thenReturn(stub);
            when(stub.getStringState("asset1"))
                    .thenReturn("{ \"assetID\": \"asset0\", \"color\": \"black\", \"size\": 14, \"owner\": \"MrX\", \"appraisedValue\": 800 }");

            Asset newAsset = contract.DuplicateAsset(ctx, "asset0", "MrX");
            assertThat(newAsset).isEqualTo(new Asset("170074T_asset0", "black", 14, "Mikan", 800));
        ...
        public void whenAssetDoesNotExist() {
            ...
            ChaincodeStub stub = mock(ChaincodeStub.class);
            when(ctx.getStub()).thenReturn(stub);
            when(stub.getStringState("asset0")).thenReturn("");

            Throwable thrown = catchThrowable(() -> {
                contract.DuplicateAsset(ctx, "asset0", "170074T_asset0", "MrX");
            });
